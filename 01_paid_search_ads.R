...
librarian::shelf(readr,psych,tm,wordcloud2,dplyr,sentimentr,syuzhet,stats,corrplot,RColorBrewer,ggplot2,openxlsx,tidyverse,scales,randomForest,text2vec,textmineR,wordcloud,ldatuning,data.table,tidytext,ldatuning,pacman,textstem,SnowballC,udpipe, quanteda, regclass, MASS, questionr, ipred, caret, dplyr, psych, survival, fitdistrplus, car, ROCR, rpart, partykit, gbm, stargazer)
...
patterns <- c("danish", "cbm", "christoffel", "wwf", "drc", "scotland", "dansk", "denmark", "keyword", "per", "near","cbm", "unicef", "red", "cross", "franke", "juventute", "switzerland", "quarrier", "caritas", "swiss", "keywords", "scottish", "quarriers", "afghanistan", "moritz", "erskine", "yemen", "inverclyde", "africa", "chesa", "syria", "hotel chesa spuondas", "james shields project", "amazon", "glasgow", "spuondas", "Franke","even", "towards", "franc", "don", "edinburgh","foremost","afghan", "lanarkshire", "mozambique","indonesia", "sulawesi", "albania", "dfh")
cleanset <- tm_map(cleanset, removeWords, patterns)
inspect(cleanset[1:10])

## Lemmatizing
cleanset_char <- sapply(cleanset, as.character)
...
lemmatize_text <- function(text) {
  ...
  x <- as.data.frame(x)
  x <- subset(x, upos %in% c("NOUN", "VERB", "ADJ", "ADV"))  # Keep only relevant parts of speech
  x <- subset(x, !is.na(lemma))  # Remove rows where lemma is missing
  x <- aggregate(lemma ~ doc_id, data = x, paste, collapse = " ")  # Collapse lemma by document
  return(x$lemma)
}

### Apply lemmatization to our cleanset.
cleanset_char <- lemmatize_text(cleanset_char)

### Apply stemming to our cleanset.
cleanset_char <- wordStem(cleanset_char)

### Convert text back to a corpus
cleanset <- Corpus(VectorSource(cleanset_char))
...
## ----Alternative for stemming and lematizing, eval=FALSE, include=FALSE-------------------------------------------------------
## patterns1 <- c("helps", "helped")
## for (pattern in patterns1) {
##   cleanset <- tm_map(cleanset, gsub, pattern = pattern, replacement = "help")
## }
## inspect(cleanset[1:5])
## 
## patterns2 <- c("saves", "saved")
## for (pattern in patterns2) {
##   cleanset <- tm_map(cleanset, gsub, pattern = pattern, replacement = "save")
## }
## inspect(cleanset[1:5])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "impairments", replacement = "impairment")
## inspect(cleanset[1:10])
## 
## patterns3 <- c("dissabilitites", "dissabilities")
## for (pattern in patterns3) {
##   cleanset <- tm_map(cleanset, gsub, pattern = pattern, replacement = "dissability")
## }
## inspect(cleanset[1:5])
## 
## patterns4 <- c("persons", "people")
## for (pattern in patterns4) {
##   cleanset <- tm_map(cleanset, gsub, pattern = pattern, replacement = "person")
## }
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "areas", replacement = "area")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "tips", replacement = "tip")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "amounts", replacement = "amount")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "children", replacement = "child")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "childs", replacement = "child")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "cataracts", replacement = "cataract")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "careers", replacement = "career")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "teenagers", replacement = "teenager")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "donations", replacement = "donation")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "charities", replacement = "charity")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "lives", replacement = "life")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "sevices", replacement = "service")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "refugees", replacement = "refugee")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "activities", replacement = "activity")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "years", replacement = "year")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "injuries", replacement = "injury")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "cards", replacement = "card")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "aliens", replacement = "alien")
## inspect(cleanset[1:10])
## 
## cleanset <- tm_map(cleanset, gsub, pattern = "rations", replacement = "ration")
## inspect(cleanset[1:10])
## 
## patterns5 <- c("experienced", "experiences")
## for (pattern in patterns5) {
##   cleanset <- tm_map(cleanset, gsub, pattern = pattern, replacement = "experience")
## }
## inspect(cleanset[1:10])
...
barplot(w, las = 2, col = rainbow(20)) #las determines the orientation, 2 = perpendicular to the axis, you can also check the documentation

# Through the bar plot, we already identify some words that need to be excluded or stemmed/lemmarized: danish, wwf,cbm, drc, scotland, etc.
...
## ----Lexicons-----------------------------------------------------------------------------------------------------------------
# Question 5 ----
# Create textual features based on three different off-the-shelf lexicons. How well do these perform?
...
ggplot(bing_top_10, aes(x = word, y = n, fill = sentiment)) +
  ...
  labs(title = "Top 10 Words by Sentiment (Bing Lexicon)",
       x = "Word",
       y = "Count") +
  scale_fill_manual(values = c("positive" = "darkmagenta", "negative" = "darkred", "neutral" = "gray")) +
  theme_minimal() +
  theme(
...
sentiment.df <- data %>%
  mutate(syuzhet = get_sentiment(Answer)) %>% #syuzhet default method
    mutate(sentimentr = sentiment_by(Answer)$ave_sentiment) %>% #sentimentr method; note: we only save column 4 (avg_sentiment)
...
## ----Topic Modeling-----------------------------------------------------------------------------------------------------------
# Question 6. ----
# Conduct topic modeling via LDA. Determine the optimal number of topics, explore, interpret, and name the topics and generate a list with the top 20 words per topic.
...
vis.df <- thetas
colnames(vis.df) <- c("Topic 1: Employment Opportunities", "Topic 2: Children and Family Support", "Topic 3: Community Services and Assistance", "Topic 4: Refugee Support")
...
temp <- as.data.frame(cbind(ID, sentiment.df[,c(3:5,7,9,12:22)])) # chose word count, clicks and sentiments.
...
analysis.df <- analysis.df [,-c(2,10:11,23,24)]
...
high_correlation_indices <- which(abs(corr_matrix) > 0.7 & corr_matrix != 1, arr.ind = TRUE)
correlated_pairs <- paste(rownames(corr_matrix)[high_correlation_indices[, 1]], 
                          colnames(corr_matrix)[high_correlation_indices[, 2]], 
                          sep = " - ")
correlation_info <- data.frame(Pair = correlated_pairs, Correlation = corr_matrix[high_correlation_indices])
print(correlation_info)
# Extremely high correlation between the 2 sentiments (sentimentr - syuzhet: 0.8388688). So we opt for keeping sentimentR, because, unlike syuzhet, it attempts to take into account valence shifters (i.e., negators, amplifiers (intensifiers), de-amplifiers (downtoners), and adversative conjunctions) while maintaining speed. Also including Bing, Afinn, NCR suffices. 
analysis.df <- analysis.df [,-8]
corr_data <- analysis.df[,-1]
corr_matrix <- cor(corr_data)
...
# First Model ----
firstmodel <- lm(clicks ~ prob_t1 + prob_t2 + prob_t3 + prob_t4 + words +
                 sentimentr + afinn + bing + anger + anticipation + 
...
## With sentiments:
inter.model1 <-  lm(formula = clicks ~ words + sentimentr + bing + anger + joy + 
                            sadness + trust + prob_t1 * sentimentr + prob_t2 * 
                            sentimentr + prob_t3 * sentimentr + prob_t4 * sentimentr, data = analysis.df[, -1])
summary(inter.model1)

## With words:
inter.model2 <-  lm(formula = clicks ~ words + sentimentr + bing + anger + joy + 
                            sadness + trust + prob_t1 * words + prob_t2 * 
                            words + prob_t3 * words + prob_t4 * words, data = analysis.df[, -1])
summary(inter.model2)
# The relationship between the number of words and the response variable (clicks) varies depending on the topic 1.
